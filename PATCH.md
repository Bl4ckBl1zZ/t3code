...
- Composer size transitions animate the fork’s existing mounted editor and compact controls, pin the overlay only for the transition, and preserve V2 timeline insets. Toolbar label widths animate without translating the whole control. Motion honors the fork’s animation preference, reduced motion, and hidden documents. Native’s full-width status band and available-width model label already avoid upstream’s floating-pill measurement issue; its persistent voice gesture hierarchy remains authoritative.

- Provider one-click updates are resolved from installer ownership for V2 Codex, Claude, and OpenCode accounts. Resolution is cached for advisories and refreshed before execution and verification; npm prefixes and native Codex shared homes are preserved. Unknown/mise-owned installs remain manual. The registry consults the current instance, not a lagging snapshot subscriber. Static/manual fork providers retain their source shape; Cursor uses the fork’s SDK and does not gain upstream’s retired CLI updater. Web settings update each account independently; Swift now reads the advisory/update state and offers account-scoped update, refresh, command copy, and results through the existing RPC. No V1 adapter or provider runtime is restored.

- Carries GitHub workflow approval and revert actions through the existing host-scoped pull-request service. Workflow discovery and each approval verify the fork head identity and retain bounded reads. Web and Swift show waiting checks, saved auto-merge strategy and reviewed actions; combined comment/state actions preserve a posted comment if the state change fails. V2 thread links, native stack actions and the fork’s motion-aware spinner remain intact. No V1 projection or migration is introduced.
