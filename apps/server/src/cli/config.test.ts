...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect(
    "resolves each signal's endpoint through T3CODE_OTLP_*_URL, an OTEL endpoint, and the bootstrap envelope, in that order",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(resolved.otlpTracesUrl).toBe("http://t3:4318/v1/traces");
        // The OTEL endpoint claimed metrics, so neither the bootstrap envelope
        // nor Settings receives them with T3 Code's headers.
        expect(resolved.otlpMetricsUrl).toBeUndefined();
      }),
  );
});
