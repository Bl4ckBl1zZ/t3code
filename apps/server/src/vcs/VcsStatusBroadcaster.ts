  ...
  Effect.gen(function* () {
    ...
    return {
      isEnabled: Effect.fn("VcsAutoPullPolicy.isEnabled")(
        function* (cwd: string) {
          const project = yield* projects.getByWorkspaceRoot(cwd);
          if (project._tag === "None") return false;
          const settings = yield* serverSettings.getSettings;
          return resolveProjectAutoPull(settings, project.value.id);
        },
...
export const make = Effect.gen(function* () {
  ...
  // Serialize remote refreshes and automatic pulls for each canonical workspace.
