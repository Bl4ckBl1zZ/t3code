...
export const make = Effect.gen(function* () {
  ...
  // macOS aliases /var to /private/var; compare both configured and canonical
  // roots so a transcript reached through a symlink cannot import T3 state.
  ...
  const scan: AgentSessionScanner["Service"]["scan"] = Effect.gen(function* () {
    ...
    const shellSnapshot = yield* projectService.snapshot.pipe(
