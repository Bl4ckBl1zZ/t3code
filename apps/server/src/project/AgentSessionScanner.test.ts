...
const makeProjectShell = (workspaceRoot: string): Project => ({
...
/** Discovery reads the surviving project aggregate; V1 thread projections are not involved. */
const makeProjectLayer = (importedWorkspaceRoots: ReadonlyArray<string>) =>
  Layer.mock(ProjectService.ProjectService)({
    snapshot: Effect.succeed({
      projects: importedWorkspaceRoots.map(makeProjectShell),
      updatedAt: "2026-01-01T00:00:00.000Z",
    }),
  });
  ...
  AgentSessionScanner.layer.pipe(
    Layer.provide(
      Layer.mergeAll(
        ...
        makeProjectLayer(input.importedWorkspaceRoots ?? []),
      ),
    ),
  );
