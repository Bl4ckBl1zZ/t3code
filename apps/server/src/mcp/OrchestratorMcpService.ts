...
/**
 * True when this page actually shows the child's untruncated terminal result.
 * Only then has the caller really read the outcome, so only then is the
 * automatic delivery redundant.
 */
...
const make = Effect.gen(function* () {
  ...
  const readTask = (
    ...
    Effect.gen(function* () {
      ...
      // this task is redundant: acknowledge it and let the cohort drop it.
      if (
        acknowledgeTerminal &&
  ...
  return OrchestratorMcpService.of({
    ...
    cancelTask: (scope, input) =>
      Effect.gen(function* () {
        ...
        // Cancelling says the caller no longer wants this task's outcome, so
        // its automatic delivery is dropped whether or not it already finished.
