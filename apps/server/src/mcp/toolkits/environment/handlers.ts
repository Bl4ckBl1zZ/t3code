...
export const EnvironmentHandlersLive = EnvironmentToolkit.toLayer({
  ...
  // shared keyed executor. Our orchestrator keeps its executor private, and a
  // second one would be a lock over nothing; `updateSettings` already does its
  // own read-modify-write under a write semaphore, which is the race that
  // matters here.
