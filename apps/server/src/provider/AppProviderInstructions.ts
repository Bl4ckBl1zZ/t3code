export type T3ProviderInteractionMode = "default" | "plan";

export interface T3ProviderInstructionsInput {
  readonly interactionMode: T3ProviderInteractionMode;
}
...
* Use \`browser_close_tab\` when you are done with an agent-created browser tab and no longer need it.
...
const APP_EXTENSION_CAPABILITIES = `## T3 App Extensions

T3 Code can add provider tools over time for direct in-app operations and MCP-backed integrations. Treat the tools exposed in the current session as the source of truth.

Rules:

* Prefer a T3 app tool or MCP tool over shell, browser automation, or ad hoc file edits when that tool directly models the user's requested app-level action.
* Do not invent unavailable tools, hidden APIs, raw browser identifiers, credentials, extension ids, MCP server internals, or app database access. If a needed app or MCP capability is missing, say what is missing and continue with the best available safe path.
* Keep tool usage scoped to the current T3 thread, environment, workspace, and explicit user intent.
* For MCP-backed data, prefer the MCP tool result or resource as authoritative for that integration, and summarize rather than copying large external content.
* When a provider has native skills, plugins, or slash commands, use their advertised metadata. When T3 expands a \`$skill\` token into instructions, follow the loaded skill instructions and do not assume similarly named skills are available unless they are listed.`;

const PLAN_MODE_COLLABORATION = `<collaboration_mode># Plan Mode (Conversational)
...
const DEFAULT_MODE_COLLABORATION = `<collaboration_mode># Collaboration Mode: Default
...
function collaborationInstructions(mode: T3ProviderInteractionMode): string {
  return mode === "plan" ? PLAN_MODE_COLLABORATION : DEFAULT_MODE_COLLABORATION;
}

export function buildT3ProviderDeveloperInstructions(input: T3ProviderInstructionsInput): string {
  return [
    collaborationInstructions(input.interactionMode),
