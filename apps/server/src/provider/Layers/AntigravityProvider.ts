...
export const makeAntigravityProvider = Effect.fn("makeAntigravityProvider")(function* (
  ...
  const snapshotForCwd = Effect.fn("AntigravityProvider.snapshotForCwd")(function* (
    ...
    const checkedAt = DateTime.formatIso(yield* DateTime.now);
    yield* SubscriptionRef.update(metadata, (state) => {
      const existing = state.draft.workspaceSnapshots?.find((entry) => entry.cwd === cwd);
      return {
        ...state,
        draft: {
          ...state.draft,
          workspaceSnapshots: [
            ...(state.draft.workspaceSnapshots ?? []).filter((entry) => entry.cwd !== cwd),
            {
              cwd,
              checkedAt,
              slashCommands: existing?.slashCommands ?? state.draft.slashCommands ?? [],
    ...
    if (skills) {
      ...
      while (discoveredSkills.size > MAX_WORKSPACE_SNAPSHOTS) {
        const oldest = discoveredSkills.keys().next().value;
        if (oldest === undefined) break;
        discoveredSkills.delete(oldest);
      }
    }
