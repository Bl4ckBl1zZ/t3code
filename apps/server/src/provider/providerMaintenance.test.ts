...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
  ...
  it("keeps mise-owned npm backends manual while recognizing Node-owned global packages", () => {
    expect(
      npmGlobalPrefixFromCommandPath(
        "/home/me/.local/share/mise/installs/npm-codex/1.0/lib/node_modules/@openai/codex/bin/codex.js",
        "@openai/codex",
      ),
    ).toBeNull();
    expect(
      npmGlobalPrefixFromCommandPath(
        "/home/me/.local/share/mise/installs/node/24.0/lib/node_modules/@openai/codex/bin/codex.js",
        "@openai/codex",
      ),
    ).toBe("/home/me/.local/share/mise/installs/node/24.0");
  });
  it.effect(
    "never upgrades Homebrew mise when a provider shim resolves to the version manager",
    () =>
      resolvePackageManagedProviderMaintenance(
        { provider: driver("codex"), npmPackageName: "@openai/codex", nativeUpdate: null },
        {
          binaryPath: "codex",
          resolvedCommandPath: "/home/me/.local/share/mise/shims/codex",
          realCommandPath: "/opt/homebrew/Cellar/mise/1.0/bin/mise",
          env: {},
          platform: "darwin",
        },
      ).pipe(
        Effect.provideService(ChildProcessSpawner.ChildProcessSpawner, noSpawn),
        Effect.map((result) => expect(result.update).toBeNull()),
      ),
  );
