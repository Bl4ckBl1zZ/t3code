// @effect-diagnostics anyUnknownInErrorContext:off, missingEffectContext:off - Effect CLI entrypoint helpers expose broad channels at the Node boundary.
...
if (import.meta.main) {
  const program = Command.run(cli, { version: packageJson.version }) as Effect.Effect<
    void,
    unknown,
    Layer.Success<typeof CliRuntimeLayer>
  >;
  program.pipe(Effect.scoped, Effect.provide(CliRuntimeLayer), NodeRuntime.runMain);
}
