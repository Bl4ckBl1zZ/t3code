...
function normalizedImageUrl(value: unknown): string | undefined {
  ...
  try {
    ...
    return url.href.length <= 4096 &&
      (url.protocol === "http:" ||
        url.protocol === "https:" ||
        (url.protocol === "data:" && /^data:image\//iu.test(url.href)))
      ? url.href
      : undefined;
  } catch {
    return undefined;
...
export function codexMcpToolPresentation(item: {
...
export function codexMcpIntentTitle(
  item: McpTitleItem,
  presentation: McpToolPresentation,
): string | undefined {
  if (normalizedMcpToolName(item.tool) === "js") {
    const title = normalizeMcpIntentTitle(asUnknownRecord(item.arguments)?.title);
    if (title) return title;
  }
  return computerUseToolTitle(item, presentation);
}
