...
it.layer(NodeServices.layer)("server self update", (it) => {
  ...
  it.effect("serializes duplicate commits behind durable marker preparation", () =>
    Effect.gen(function* () {
        ...
        selfUpdate: {
          update: () =>
            Effect.succeed({
              targetVersion: "1.2.0",
              method: "desktop-app" as const,
              desktopUpdateToken: "serial",
            }),
          commitDesktopUpdate: () =>
            Effect.sync(() => {
              commits++;
            }).pipe(Effect.andThen(new ServerSelfUpdateError({ reason: "install failed" }))),
        },
        prepare: Deferred.succeed(started, undefined).pipe(
          Effect.andThen(Deferred.await(release)),
          Effect.as(["run-marker"]),
        ),
        clear: () => Effect.void,
      });
      yield* selfUpdate.update({ targetVersion: "1.2.0", continueRunningThreads: true });
      const first = yield* selfUpdate
        .commitDesktopUpdate("serial")
        .pipe(Effect.exit, Effect.forkChild);
      yield* Deferred.await(started);
      const second = yield* Effect.forkChild(
        selfUpdate.commitDesktopUpdate("serial").pipe(Effect.exit),
  ...
  it.effect("does not mark threads for expired desktop preparations", () =>
    Effect.gen(function* () {
      let preparations = 0;
      const selfUpdate = yield* ServerSelfUpdate.withRunningThreadContinuation({
        mode: "desktop",
        selfUpdate: {
          update: () =>
            Effect.succeed({
              targetVersion: "1.2.0",
              method: "desktop-app" as const,
              desktopUpdateToken: "expired",
            }),
          commitDesktopUpdate: () => new ServerSelfUpdateError({ reason: "token expired" }),
        },
        prepare: Effect.sync(() => {
          preparations++;
          return ["run-marker"];
      ...
      yield* selfUpdate.commitDesktopUpdate("expired").pipe(Effect.flip);
