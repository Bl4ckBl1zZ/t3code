...
const encodePromptJson = JSON.stringify;
...
/** Adds bounded capture data at the V2 provider boundary without altering the durable user message. */
export function appendSnapShotContext(
  text: string,
  ...
  const appendAttachmentContext = (context: string | undefined) => {
    ...
    if (candidate.length <= PROVIDER_SEND_TURN_MAX_INPUT_CHARS) result = candidate;
  };
