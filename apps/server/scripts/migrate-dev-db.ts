 ...
 *      threads that have fully stopped. Threads with active runs, live
 *      provider sessions, or settled state are skipped so the dev server
 *      never adopts live work. Auth sessions, pairing links, command
 *      receipts, effect outboxes, and provider runtime rows are dropped —
 *      pair a fresh browser against dev.
 ...
 *
 * The fork runs threads on orchestration V2: thread state lives in
 * `orchestration_v2_*` tables (JSON projections plus the
 * `orchestration_v2_events` log), while `orchestration_events` carries only
 * the project aggregate and legacy imports. Both event logs are pruned per
 * stream while `sqlite_sequence`, `projection_state`, and
 * `orchestration_v2_projection_metadata` carry over untouched, so new
...
/** V2 projection tables that key rows by thread_id. Rows whose thread is
 * pruned go with it; rows with a NULL thread_id (pooled provider sessions)
 * survive the NOT IN filter and are left alone. */
const V2_THREAD_SCOPED_TABLES = [
  ...
  "hermes_session_imports",
  "checkpoint_diff_blobs",
] as const;

/** Legacy V1 projection tables the fork keeps for the import path; pruned
 * by the same kept-thread set so imported and live data stay consistent. */
const V1_THREAD_SCOPED_TABLES = [
  "projection_threads",
...
const pruneSnapshot = Effect.fn("pruneDevDbSnapshot")(function* (input: RunMigrateDevDbInput) {
  ...
  // "Stopped" on orchestration V2: the thread is live (not archived or
  // deleted), not settled (settled threads may still own monitored
  // background work conceptually parked), has no run in an active status,
  // and owns no live provider session (an idle-but-ready session can host
  // monitors and dev servers).
    ...
    WHERE t.deleted_at IS NULL
      ...
      AND NOT EXISTS (
        ...
        WHERE s.thread_id = t.thread_id
          AND s.status NOT IN ('stopped', 'error')
      )`).unprepared;
  ...
  yield* sql.withTransaction(
    Effect.gen(function* () {
      ...
      for (const table of [...V2_THREAD_SCOPED_TABLES, ...V1_THREAD_SCOPED_TABLES]) {
        yield* sql.unsafe(
      ...
      yield* sql`DELETE FROM orchestration_v2_projection_context_handoffs
        WHERE thread_id NOT IN (SELECT thread_id FROM kept_threads)`;
      // Cross-thread rows go when either side goes.
      ...
      // The V2 event log is per-thread; the V1 log carries the project
      // aggregate plus legacy thread streams.
      yield* sql`DELETE FROM orchestration_v2_events
        WHERE thread_id NOT IN (SELECT thread_id FROM kept_threads)`;
      ...
      // Live-server state never transfers: receipts, queued side effects,
      // provider runtime rows, and auth all restart from scratch in dev.
      yield* sql`DELETE FROM orchestration_command_receipts`;
      yield* sql`DELETE FROM orchestration_v2_command_receipts`;
  ...
  const [events] = yield* sql<{ count: number }>`
    SELECT
      ...
      + (SELECT COUNT(*) FROM orchestration_events) AS count`;
