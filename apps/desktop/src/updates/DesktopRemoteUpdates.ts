...
> = Effect.gen(function* () {
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* updates.deferAutomaticInstall(
          request.requestId,
          now + Duration.toMillis(Duration.minutes(25)),
        );
      ...
      Effect.ensuring(
        Effect.all(
          [
            Ref.get(preparedUpdateRef).pipe(
              Effect.flatMap((prepared) =>
                Option.isSome(prepared) &&
                prepared.value.requestId === request.requestId &&
                prepared.value.status === "prepared"
                  ? Effect.void
                  : updates.deferAutomaticInstall(request.requestId, null),
              ),
            ),
  ...
  yield* Stream.runForEach(publisher.updateCancellations, (cancellation) =>
    Effect.gen(function* () {
      yield* updates.deferAutomaticInstall(cancellation.requestId, null);
  ...
  yield* Stream.runForEach(publisher.updateCommits, (commit) =>
    Effect.gen(function* () {
      ...
      Effect.ensuring(
        clearActiveRequest(commit.requestId).pipe(
          Effect.andThen(updates.deferAutomaticInstall(commit.requestId, null)),
        ),
      ),
      Effect.catchCause((cause) =>
