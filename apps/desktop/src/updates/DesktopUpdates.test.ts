  ...
  it.effect("defers the automatic install until backends report no agent activity", () => {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        activeRunCount = 0;
        // The periodic check now runs on top of a downloaded update instead of
        // being skipped, and it holds the single update-action lock while it
        // runs. Settle it so the idle watcher can take the lock and install.
  ...
  it.effect("stops waiting for idle when the auto-update preference is turned off", () => {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        // The startup poll runs on top of the downloaded update now; settling it
        // returns the state to "downloaded" instead of discarding the download.
