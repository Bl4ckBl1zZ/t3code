...
export class DesktopUpdates extends Context.Service<
  DesktopUpdates,
  {
    ...
    readonly deferAutomaticInstall: (
      requestId: string,
      until: number | null,
    ) => Effect.Effect<void>;
...
export const make = Effect.gen(function* () {
  ...
  const automaticInstallHolds = yield* Ref.make<ReadonlyMap<string, number>>(new Map());
  ...
  const checkForUpdates = Effect.fn("desktop.updates.checkForUpdates")(function* (
    ...
    // A queued automatic install is a decision already made: the binary is on
    // disk and the watcher is only waiting for the machine to go idle. A check
    // would move the status off "downloaded" and cancel that wait, so a
    // pending auto-install blocks a check the same way an active download does.
    if (state.status === "downloading" || state.autoInstallPending) {
      yield* logUpdaterInfo("skipping update check while update is active", {
        reason,
        status: state.status,
        autoInstallPending: state.autoInstallPending,
      });
      return false;
    }
  ...
  const autoInstallWhenIdle = Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      while (true) {
        ...
        const holds = yield* Ref.modify(automaticInstallHolds, (current) => {
          const live = new Map([...current].filter(([, until]) => until > now));
          return [live.size > 0, live] as const;
        });
        if (holds) {
  ...
  return DesktopUpdates.of({
    ...
    deferAutomaticInstall: (requestId, until) =>
      Ref.update(automaticInstallHolds, (current) => {
        const next = new Map(current);
        if (until === null) next.delete(requestId);
        else next.set(requestId, until);
        return next;
      }),
