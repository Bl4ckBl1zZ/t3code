...
import {
  ...
  desktopUpdateHoldsDownloadedUpdate,
...
export const make = Effect.gen(function* () {
  ...
  const autoInstallWhenIdle = Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      while (true) {
        const state = yield* Ref.get(updateStateRef);
        if (!desktopUpdateHoldsDownloadedUpdate(state) || (yield* Ref.get(desktopState.quitting))) {
          break;
        }
        if (!(yield* desktopSettings.get).autoUpdateEnabled) {
          break;
        }
        // A check in flight holds the single update-action lock and leaves the
        // status on "checking"; installing has to wait for it to settle.
        if (state.status === "downloaded" && !(yield* hasAnyAgentActivity)) {
    ...
    yield* decodeUpdateInfo(raw).pipe(
      Effect.flatMap(
        Effect.fn("desktop.updates.applyUpdateAvailable")(function* (info) {
          ...
          if ((yield* desktopSettings.get).autoUpdateEnabled) {
            if (state.downloadedVersion === info.version) {
            } else {
