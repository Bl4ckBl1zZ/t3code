    ...
    yield* Effect.gen(function* () {
      ...
      if (keySequence.commands?.length) {
        ...
        const clipboardData = keySequence.commands.includes("paste")
          ? // Electron 43 has no async clipboard.read(); collect the same MIME
            // formats through the synchronous per-format readers instead.
            yield* attempt(context, () => {
              const formats: Array<{ type: string; data: string }> = [];
              let hasImage = false;
              for (const type of clipboard.availableFormats()) {
                if (type === "text/plain") formats.push({ type, data: clipboard.readText() });
                else if (type === "text/html") formats.push({ type, data: clipboard.readHTML() });
                else if (type === "text/rtf") formats.push({ type, data: clipboard.readRTF() });
                else if (type.startsWith("image/")) hasImage = true;
              }
              if (hasImage) {
                const image = clipboard.readImage();
                if (!image.isEmpty()) {
                  formats.push({ type: "image/png", data: image.toPNG().toString("base64") });
                }
              }
              return formats;
            })
