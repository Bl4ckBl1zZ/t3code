...
export class GhosttyTerminalSurface {
  ...
  private readonly onKeyDown = (event: KeyboardEvent) => {
    ...
    // No selection: copy the whole scrollback, matching the iOS client. Only
    // the copy-only chords qualify — a plain Ctrl+C on non-mac is SIGINT and
    // has to reach the shell. The transient select-all never renders; it is
    // cleared synchronously, before the next animation frame.
    if (isTerminalCopyShortcut(event) && (event.shiftKey || isMacPlatform(navigator.platform))) {
      event.preventDefault();
      this.suppressedKeyCodes.add(event.code);
      if (this.core.selectAll() !== null) {
        const text = this.core.selectionText();
        this.core.clearSelection();
        // No DOM selection backs the canvas, so there is no native copy event
        // to race here: write straight to the clipboard.
