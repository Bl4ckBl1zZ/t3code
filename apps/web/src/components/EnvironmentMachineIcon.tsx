...
const environmentMachineAtom = Atom.family((environmentId: EnvironmentId | null) =>
  Atom.make((get) =>
    resolveEnvironmentMachineKind(
...
/** Subscribe only to the resolved glyph, so quota/config updates do not repaint thread rows. */
export function ConnectedEnvironmentMachineIcon({
  environmentId,
  ...props
}: LucideProps & { environmentId: EnvironmentId | null }) {
  const kind = useAtomValue(environmentMachineAtom(environmentId));
  return <EnvironmentMachineIcon kind={kind} {...props} />;
}
