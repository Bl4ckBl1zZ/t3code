...
export default function ChatView(props: ChatViewProps) {
  ...
  const setFileTabDirty = useWorkbenchStore((state) => state.setFileTabDirty);
  const chatWorkbenchTabs = useMemo(
    () =>
      threadTabs.map(
        (tab): WorkbenchTab => ({
          kind: "chat",
  ...
  const closeWorkbenchTab = useCallback(
    (tab: WorkbenchTab) => {
      if (tab.kind === "chat") {
        const threadTab = threadTabs.find(
          (candidate) => scopedThreadKey(candidate.threadRef) === scopedThreadKey(tab.threadRef),
        );
        if (threadTab) {
      ...
      if (tab.kind === "file") {
        if (tab.dirty && !window.confirm(`Close ${tab.relativePath} with unsaved changes?`)) {
          return;
        }
        closeFileTab(tab.id);
      }
    },
    [closeFileTab, closeThreadTab, threadTabs],
  );
                ...
                {isGitRepo && !browserAgentSidebarMode && (
                  <BranchToolbar
                    ...
                    {...(canOverrideServerThreadEnvMode
                      ...
                      : {})}
                    {...(canOverrideServerThreadEnvMode
                      ? { worktreeModeOverride: worktreeMode }
                      : {})}
                    {...(canOverrideServerThreadEnvMode
                      ? {
