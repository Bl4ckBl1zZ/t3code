...
import { createComposerReadingGesture, recordComposerReadingGesture } from "./composerRestingState";
...
describe("timeline scroll targets", () => {
  ...
  it("does not accumulate nested scrolling toward composer collapse", () => {
    const { timeline, group, content } = setup();
    const state = createComposerReadingGesture();
    const record = (target: ScrollElement, now: number, delta: number) =>
      recordComposerReadingGesture(state, {
        now,
        delta,
        eligible: targetsTimeline(target, timeline, -delta),
        canScroll: timeline.scrollTop > 0,
        towardLogicalEnd: false,
      });
