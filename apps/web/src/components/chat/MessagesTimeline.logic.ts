 ...
 * narrow pane, or a wider Chat width setting. A fixed-width hover strip would
 * then sit on top of the message text and swallow its pointer events. Cap the strip's width so it never
...
function deriveTurnFolds(input: {
  ...
  for (const [runId, group] of groupsByRunId) {
    ...
    // Anchored at the first *hidden* entry, not the first entry: the opening
    // assistant message stays visible above the fold.
