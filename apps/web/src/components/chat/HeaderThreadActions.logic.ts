...
export interface ThreadActionMenuState {
  ...
  readonly supports: {
    ...
    /** Server understands autoSettle on thread.metadata.update. */
...
/**
 * Header action presentation. Mutations remain in the fork's useThreadActions
 * alongside the sidebar, while this surface derives its menu from a fresh V2 shell.
 */
export function buildThreadActionMenuItems(
  ...
  return [
    ...
    // A submenu with the current option checked, not a one-shot action: this
    // is a setting, and it sits with the other per-thread settings rather
    // than the lifecycle verbs above. Disabled keeps long-running threads out
    // of the settled shelf no matter how quiet they get.
