...
} from "./HeaderThreadActions.logic";
...
it("disables settlement for a V2 run that still needs attention", () => {
  expect(
    buildThreadActionMenuItems({ ...baseState, canSettleNow: false }).find(
      (item) => item.id === "settle",
    )?.disabled,
  ).toBe(true);
});
