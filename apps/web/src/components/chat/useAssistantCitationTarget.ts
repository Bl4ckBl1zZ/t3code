...
export function useAssistantCitationTarget({
  ...
  onExpandTurn: (runId: RunId | null, attemptId: RunAttemptId | null) => boolean;
  ...
  useEffect(() => {
    ...
    if (index < 0) {
      if (!onExpandTurn(source.message.runId ?? null, source.attempt?.id ?? null)) {
        fail(
          "The cited response is hidden",
          "The response is outside this timeline view. Your saved quote is unchanged.",
        );
      }
      return;
    }
