...
export function ThreadPreviewMiniPlayer({ threadRef, tabId, bottomInset }: Props) {
  ...
  useLayoutEffect(() => {
    const clampAndMove = () => {
      ...
      if (!position) {
        // A player that has never been placed lands under the thread-details
        // card rather than on top of it. Materializing that here also gives
        // the overlay a stable anchor; the version still changes so a parent
        // resize (the panel closing) re-measures it.
        usePreviewMiniPlayerStore
          ...
          .move(
            ...
            tabId,
            clampPreviewMiniPlayerPosition(
              initialPreviewMiniPlayerPosition(root, parent),
              { width: parent.clientWidth, height: parent.clientHeight },
              nextSize,
              bottomInset,
            ),
