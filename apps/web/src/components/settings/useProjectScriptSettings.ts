...
export function useProjectScriptSettings(
  ...
  const updateProject = useAtomCommand(projectEnvironment.update, "project actions legacy update");
  ...
  async function persist(
    ...
    try {
      for (const {
        ...
        const result =
          !supportsDefaults && project
            ? await updateProject({
                environmentId,
                input: { projectId: project.id, scripts: effectiveScripts },
              })
