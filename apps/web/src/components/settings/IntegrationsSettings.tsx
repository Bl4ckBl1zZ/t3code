 ...
 * state their own size. OpenRouter follows: the account-wide credential Voice
 * Input routes through, managed on its own sub-page.
 *
...
export function IntegrationsSettings() {
  ...
  useEffect(() => {
    void getOpenRouterIntegration()
      .then(setOpenRouterStatus)
      .catch(() => setOpenRouterStatus(null));
  }, []);
    ...
    <SettingsPageContainer>
      ...
      <SettingsSection {...searchableSetting("integrations")}>
        <SettingsRow
          ...
          status={`${statusLabel(openRouterStatus)} · Used by Voice Input`}
          control={
            <Button size="sm" render={<Link to="/settings/integrations/openrouter" />}>
              {openRouterStatus?.configured ? "Manage" : "Configure"}
            </Button>
          }
        />
      </SettingsSection>
