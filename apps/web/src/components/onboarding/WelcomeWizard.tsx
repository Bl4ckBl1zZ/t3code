  ...
  return (
    <>
      ...
      <div className="mt-4 space-y-2">
        {cloudEnabled ? <ConnectAccountOption disabled={isPairing} /> : null}
...
function ConnectAccountOption({ disabled }: { readonly disabled: boolean }) {
  const { environments } = useEnvironments();
  ...
  return (
    <div className="rounded-lg border border-border bg-background">
      <Collapsible open={expanded && !!isSignedIn} onOpenChange={setExpanded}>
        <CollapsibleTrigger
        ...
        >
          ...
          <span className="text-xs text-muted-foreground">
            {!isLoaded ? "Loading sign-in…" : !isSignedIn ? "Sign in" : null}
          </span>
          <ChevronRightIcon
...
function ConnectedAgentsStep({
  ...
  return (
    <section>
      ...
      serverConfig.environment.capabilities.providerTerminalEnvironment !== true ? (
        <p className="mt-3 text-xs text-muted-foreground">
          Update this server to install or sign in to a specific agent account here.
        </p>
      ) : null}
