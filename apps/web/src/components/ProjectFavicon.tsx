import type { EnvironmentId, ProjectId, ProjectIconColor } from "@t3tools/contracts";
...
// Explicit project records win; legacy row surfaces resolve the canonical scoped
// record from project state, never their shortened display label.
...
export function ProjectFavicon(input: {
  ...
  project?: ProjectFaviconProject | undefined;
  ...
  const project = input.project ??
    projects.find(
      ...
      title: input.cwd.split(/[\\/]/).filter(Boolean).at(-1) ?? "Project",
      faviconPath: input.faviconPath,
      projectIcon: null,
    };
  const asset = useProjectFaviconAsset({
  ...
  const cacheKey = getProjectFaviconCacheKey(project.environmentId, project.workspaceRoot, src);
...
function ProjectFaviconImage({
  ...
  const [displayedSrc, setDisplayedSrc] = useState<string | null>(
    () => loadedProjectFaviconSrcs.get(cacheKey) ?? (src.startsWith("data:image/") ? src : null),
  );
