...
interface ChatMarkdownComponentsContext {
  ...
  /** False when the environment cannot run shell actions on this machine, which
      withholds the editor-open and reveal actions from every file chip. */
  ...
  /** Platform-specific reveal wording, or undefined when the host advertises no
      usable file manager. */
  readonly revealInFileManagerLabel: string | undefined;
  readonly openFileInPanel: (workspaceRelativePath: string, line: number | undefined) => void;
  ...
  readonly openChangeRequestLink: (event: React.MouseEvent<HTMLElement>, href: string) => boolean;
  /** Null when this link is not a PR the thread's environment can link to. */
      ...
      if (!fileLinkMeta) {
        ...
        return (
          <Tooltip>
            ...
            <TooltipPopup side="top">{unresolvedPathHref ?? href}</TooltipPopup>
          </Tooltip>
        );
      }
