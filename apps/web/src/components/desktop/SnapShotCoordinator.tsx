...
export async function deliverSnapShot(
  ...
  if (!alreadyAttached) {
    if ((store.getComposerDraft(target)?.images.length ?? 0) >= PROVIDER_SEND_TURN_MAX_ATTACHMENTS)
    ...
    if (
      !useComposerDraftStore
        ...
        ?.images.some(({ id }) => id === capture.id)
    )
      throw new Error("The destination draft is no longer available.");
  }
