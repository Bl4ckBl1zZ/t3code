...
function sessionResponse(session: AuthSessionState): Response {
  return jsonResponse(encodeAuthSessionState(session));
}
...
describe("resolveInitialServerAuthGateState", () => {
  ...
  it("uses a browser transfer pairing token before route redirects can drop it", async () => {
    ...
    expect(testApi.calls.browserSession).toEqual([{ credential: "pairing-token" }]);
  });
  ...
  it("accepts same-session bearer tokens for browser-agent sidebar chat frames", async () => {
    ...
    expect(String(fetchMock.mock.calls[0]?.[0])).toBe("http://localhost/api/auth/session");
