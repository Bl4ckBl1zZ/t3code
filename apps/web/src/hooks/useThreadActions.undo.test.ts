...
describe("settle and snooze Undo", () => {
  ...
  // Orchestration V2's settle drops the pin but keeps the order key and any
  // snooze, so Undo re-pins in place and never re-snoozes.
  it("re-pins a thread that settling had unpinned and leaves its snooze alone", async () => {
  ...
  // Orchestration V2's snooze also drops the pin, so waking has to restore it.
  it("re-pins a thread that snoozing had unpinned", async () => {
    threadShell.pinnedAt = "2026-01-01T00:00:00.000Z";
    const actions = useThreadActions();
    await actions.snoozeThread(target, new Date(Date.now() + 60_000).toISOString());
    await currentUndo()();
  ...
  it("expires an older unpin Undo when the thread is snoozed", async () => {
    const actions = useThreadActions();
    await actions.unpinThread(target);
    const staleUnpinUndo = currentUndo();
    await actions.snoozeThread(target, new Date(Date.now() + 60_000).toISOString());
