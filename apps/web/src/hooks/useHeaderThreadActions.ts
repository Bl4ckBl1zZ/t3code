...
} from "../components/chat/HeaderThreadActions.logic";
...
import { useThreadActions, useMarkThreadUnread } from "./useThreadActions";
 ...
 */
export function useHeaderThreadActions(input: {
    ...
    (position: { x: number; y: number }) => {
      ...
      void (async () => {
        ...
        const supports = {
          ...
          pinning:
            thread.workInboxRole !== "main" &&
            readEnvironmentSupportsPinning(threadRef.environmentId),
          titleRegeneration: readEnvironmentSupportsTitleRegeneration(threadRef.environmentId),
        };
        const snapshot = appAtomRegistry
          .get(threadChangeRequestSnapshotsAtom)
          .get(scopedThreadKey(threadRef));
        const changeRequest =
          snapshot != null &&
