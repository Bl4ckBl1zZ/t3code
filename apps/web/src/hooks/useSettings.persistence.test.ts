...
describe("persistClientSettingsPatch", () => {
  it("waits for settings writes in request order", async () => {
    ...
    const firstSettings = { ...DEFAULT_CLIENT_SETTINGS, wordWrap: false };
    const secondSettings = { ...firstSettings, composerCollapseOnScroll: false };
    const first = persistClientSettingsPatch({ wordWrap: false });
    await firstStarted;
    const second = persistClientSettingsPatch({ composerCollapseOnScroll: false });
...
describe("persistClientSettingsUpdate", () => {
  ...
  it("preserves an optimistic write made while an awaited update persists", async () => {
    ...
    __setClientSettingsForTests(DEFAULT_CLIENT_SETTINGS);
    const importedProfile = { provider: ProviderInstanceId.make("codex_work"), model: "gpt-5.4" };
