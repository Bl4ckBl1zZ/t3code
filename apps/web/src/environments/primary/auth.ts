...
function writePrimaryBrowserAgentSidebarSessionToken(token: string): void {
  try {
    ...
    // Session storage is best-effort; bootstrap still succeeds for the current load.
  }
}
...
async function acceptBrowserAgentSidebarSessionToken(
  ...
  try {
    ...
    if (!result.authenticated || !result.expiresAt || !result.sessionMethod) {
      throw new BootstrapHttpError({
    ...
    throw new BootstrapHttpError({
      message: message || `Failed to verify browser sidebar session (${status}).`,
      status,
    });
  }
}
...
async function exchangeBootstrapCredential(credential: string): Promise<AuthBrowserSessionResult> {
  if (isBrowserAgentSidebarMode()) {
    return retryTransientBootstrap(async () => acceptBrowserAgentSidebarSessionToken(credential));
  }
...
async function bootstrapServerAuth(): Promise<ServerAuthGateState> {
  ...
  if (urlCredential) {
    try {
      await exchangeBootstrapCredential(urlCredential);
