...
/** Tuple encoding preserves arbitrary environment IDs without delimiter collisions. */
const configKey = (environmentId: EnvironmentId, profileId: string | undefined): string =>
  JSON.stringify([environmentId, profileId ?? "default"]);

const parseConfigKey = (key: string): { environmentId: EnvironmentId; profileId: string } => {
  const [environmentId, profileId] = JSON.parse(key) as [EnvironmentId, string];
