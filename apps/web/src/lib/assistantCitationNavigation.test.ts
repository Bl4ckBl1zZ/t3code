...
describe("assistant citation navigation", () => {
  it("supports maximum-length Unicode quotes and comments in the route fragment", () => {
    const large = {
      ...citation,
      text: "引".repeat(8_000),
      comment: "注".repeat(8_000),
      environmentId: EnvironmentId.make("環".repeat(512)),
      threadId: ThreadId.make("線".repeat(512)),
      messageId: MessageId.make("文".repeat(512)),
    };
    const navigation = assistantCitationNavigation(large);
    expect(assistantCitationFromLocation(`/a/b#${navigation.hash}`)).toEqual(large);
  });
