...
describe("buildFileDiffRenderKey", () => {
  ...
  it("gives each block of a file-to-symlink type change its own key", () => {
    const patch = [
    ...
    const uncached = parsed.files.map(({ cacheKey: _cacheKey, ...file }) => file);
    expect(new Set(uncached.map(buildFileDiffRenderKey)).size).toBe(uncached.length);
  });
});
