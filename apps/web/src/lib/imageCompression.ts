...
export type CompressImageFileResult =
  | { ok: true; file: File; recompressed: boolean; imageSize?: { width: number; height: number } }
  | { ok: false; reason: ImageCompressionFailureReason };
...
type ReencodeResult =
  | { ok: true; dataUrl: string; mimeType: string; imageSize: { width: number; height: number } }
  | { ok: false; reason: ImageCompressionFailureReason };
