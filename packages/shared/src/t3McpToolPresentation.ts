...
export type T3McpToolLogo = "t3-code" | "pull-request" | "browser" | "device";
...
/**
 * Render a T3 tool row: the provider's status drives the verb, and a linked
 * pull request resolves to its number so a row reads "Linked PR #42".
 */
export function resolveT3McpToolPresentation(
  ...
  return {
    displayName:
      status === undefined && input === undefined ? definition.displayName : `${verb} ${target}`,
    logo: definition.icon,
