 ...
 * blocks it for good. Falls back to creation time for untouched threads, and
 * to null for shells that carry neither (older projections), which keeps the
...
function threadUserActivityAnchorAt(thread: ThreadActivitySource): string | null {
  ...
  for (const candidate of [messageAt, requestedAt]) {
    ...
    if (anchor === null || Date.parse(candidate) > Date.parse(anchor)) {
      anchor = candidate;
    }
  }
  return anchor;
...
export function threadLastActivityAt(shell: QueuedThreadShell): string | null {
