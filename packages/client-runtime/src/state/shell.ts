  ...
  yield* Effect.forkScoped(
    subscribeDynamic(
      ...
      Effect.fn("EnvironmentShellState.makeSubscribeInput")(function* (session) {
        ...
        let current = yield* Ref.get(latestLiveSnapshot);
        if (!hasAuthoritativeSnapshot || Option.isNone(current)) {
          ...
          if (Option.isSome(httpSnapshot)) {
            ...
            canResume = true;
            current = yield* Ref.get(latestLiveSnapshot);
          }
        }
        ...
        if (!canResume || Option.isNone(current)) {
        ...
        return {
          afterSequence: current.value.snapshotSequence,
