...
- **Projects & Worktrees**: Project registry, repository identity, branch/worktree preparation, setup scripts, per-project settings, archived workspace cleanup, and unlinked worktree management.
...
- Browser routing policy: provider/agent browser tabs use the browser-agent context. User-initiated Preview uses the browser-agent extension in the same browser where the user clicked Preview, after rewriting loopback preview URLs to a reachable active-environment host when needed. Same-origin Preview may mint a browser-agent session from the current browser session, then target the connected extension by `preferredAgentId`; when the fresh session has not registered yet, only a non-local-control connected extension may be used as fallback. Preview must not create manual browser-agent pairing tokens or fall back to a host-paired local-control extension. Remote app loads over non-loopback origins such as Tailscale/LAN must not use build-time loopback backend URLs; the primary environment target should fall back to the current app origin.
...
- Browser-agent host control should not require manual extension pairing. Same-machine desktop control uses unauthenticated `GET /browser-agent/local-ws`, which must stay desktop-mode and loopback-only. Authenticated remote/manual pairing remains on `GET /browser-agent/ws`; token auto-connect via `POST /browser-agent/auto-connect` is fallback behavior and must issue client bearer sessions through `ServerAuth`. Same-origin remote browser clients use authenticated `browserAgents.session.issue` RPC first to mint a bearer session from the active app connection, then pass that session to the extension content script; page-side `POST /browser-agent/session` is only a compatibility fallback. Preview RPCs target that extension with `preferredAgentId` / `preferredSessionId` instead of falling back to the host browser. If content-script pairing is unavailable after a browser-agent session is issued, manual setup links may carry the short-lived session token in the URL hash so the setup page does not need to mint another session from cookies. Extension side-panel chat URLs use short-lived sidebar bearer tokens; keep tokens out of stored workspace-link URLs and use extension session storage for refresh/reopen reconstruction.
...
- `apps/web/src/browserAgentPairing.ts`: Browser-agent setup helpers; same-origin Preview mints `/browser-agent/session`, requests the content script to pair, and returns a connected `preferredAgentId` while excluding local-control fallback.
