...
The relay Worker reads its variables and secrets when it is deployed. Alchemy does not redeploy the
...
3. Set `dry-run`, run a forced relay deploy, and read the sweep counters (`scanned`, `wouldDelete`,
   `skippedLegacy`, `skippedOrphan`, `failed`, `truncated`) across several sweeps. Each sweep logs
   them as "Finished managed tunnel cleanup" in the Worker logs.
