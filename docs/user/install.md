...
**Settings → Providers** on web and desktop, or the account's configuration in iOS **Settings →
Agents**, for the recommended version or range. When its package manager supports installing a
specific version, you can install the recommendation there. Otherwise use the provider's installer
